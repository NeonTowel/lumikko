<script>
  ...
  async function sanitize() {
    ...
    try {
      ...
      const { auth } = await import("$lib/authService");
      const token = await auth.getAccessToken();

      const res = await fetch(
        `${env.PUBLIC_API_URL}/api/generate_with_recommendations`,
        {
          method: "POST",
          headers: {
  ...
  <div class="flex-1 flex flex-col">
    <div class="w-full bg-gray-800 bg-opacity-90 flex flex-col gap-4 flex-1">
      <div class="pt-12 pb-2">
        ...
        <p class="text-xl font-semibold text-gray-300 text-center mb-4">
      ...
      <div class="flex flex-row gap-4 px-4 pb-6 flex-1">
        <div class="flex-1 flex flex-col min-w-0">
          ...
          <pre
            class="whitespace-pre-wrap bg-gray-900 p-4 rounded-lg overflow-x-auto text-lg font-mono text-gray-400 border border-gray-700 min-h-[200px]">{code}</pre>
        </div>
        <div class="flex-1 flex flex-col min-w-0">
          <div class="text-lg font-semibold text-purple-300 mb-2">Improved</div>
          <pre
            class="whitespace-pre-wrap bg-gray-900 p-4 rounded-lg overflow-x-auto text-lg font-mono text-gray-100 border border-purple-700 min-h-[200px]">{sanitizedCode}</pre>
        </div>
      </div>
    </div>
    ...
    >
      {#if recommendations}
        <div
          class="w-full sm:w-[100%] lg:w-[96%] xl:w-[84%] 2xl:w-[72%] max-w-5xl mx-auto mb-4 p-4 px-8 rounded-lg bg-[#1a0826] bg-opacity-90 border border-purple-950 shadow-lg"
        >
          <div class="mb-2 text-purple-200 font-semibold">
            Recommendations for further code improvements:
          </div>
          <div
            class="prose prose-invert prose-purple max-w-none font-normal text-gray-200
